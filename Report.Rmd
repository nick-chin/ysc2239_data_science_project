...
Seems that there is some form of linearity between gini index and proportion of those with a highschool degree & poportion of the population that is not white. We decided that we will want to find a model that has good predictive power of the gini index. As such, we decided to approach it using lasso regression. We also ignore predictors `splc_hate_crimes` and `fbi_hate_crimes` as the article have already concluded that income inequality is predictive of hate crimes. As we want to explore the what predictors are good predictors of income inequality, it would be sensible to ignore these two varaibles. We also ignore `state` as a predictor varaible as the name of the state should not be included in the model.

We begin by performing the lasso regression with `gini_index` as the outcome varaible. However, `cv.glmnet()` only works when the matrix model and the data frame has the same amount of rows. In other words, it only works when the data has no missing values. As such, we would have to consider only the complete cases.
...
From the lasso regression results, we see that it selects 4 predictor varaibles. Namely, it selects `prop_unemployed, prop_in_metro, prop_hs_degree, prop_trump_vote`. Now we can create the multivariate linear model and check for linear regression assumptions.
...
For a linear regression, we need to check 4 different assumptions.
  ...
  1. Linearity
    + Looking at the Residuals vs. Fitted plot, we can see that the data is fairly linearly related, as the red line is mostly along the horizontal 0 line. The only part that is questionable is at high fitted values, where the red line drops down significantly.
...
From this, we can see that the model has an adjusted R-squared value of 0.5812. This means that the linear mode explains roughly 58% of the varaince in the data. Overall, it is moderate in predicting gini index given the predictor varaibles.
